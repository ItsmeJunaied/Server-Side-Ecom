  ...
  try {
    ...
    app.post('/video', upload.single('video'), async (req, res) => {
      try {
        ...
        // Store necessary video information in the database
        const fileInfo = {
          title: req.body.title,
          url: result.secure_url, // Cloudinary URL of the uploaded video
          public_id: result.public_id, // Public ID of the uploaded video (if needed)
          type: result.resource_type,
          space: result.bytes
        };
    
        // Save the data to MongoDB
        const insertedVideo = await videoData.insertOne({ fileInfo, status: 'Inactive' });
    
        // Respond with success message and inserted video ID
        res.json({ message: 'Video added successfully', videoId: insertedVideo.insertedId });
      } catch (error) {
    ...
    app.post('/orders', async (req, res) => {
      ...
      if (localCartData && localCartData.length > 0) {
        ...
        const data = {
          ...
          success_url: `https://taharz.onrender.com/payment/success/${tran_id}`,
          fail_url: `https://taharz.onrender.com/payment/fail/${tran_id}`,
